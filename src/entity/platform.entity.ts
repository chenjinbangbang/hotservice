...
export class Platform extends BaseEntity {
  ...
  @Column({ type: 'tinyint', comment: '平台类型（0：今日头条 1：抖音短视频 2：火山小视频，3：快手）', default: 0, update: false })
  ...
  @Column({ type: 'tinyint', comment: '平台账号审核不通过原因（0：平台不存在该账号 1：平台账号头像与账号不对应 2：平台账号截图和填写的信息不对应）（平台账号审核/冻结状态为1时使用）' })
  reason: number;
